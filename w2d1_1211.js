...
function consultTheOracle(arr) {
    // code here
    var pickedNum = Math.floor(Math.random()*arr.length);//arr.length is 20 and use random will give you a random number between 0 and 1 exculsive.
    return arr[pickedNum];
}
...
function concatArrWithSelf(items) {
    // code here
    var arrNew=[];//can't directly assign temp = items
    for (var i = 0; i < 2; i++){//run the loop twice
